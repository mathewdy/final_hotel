...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Executive Rooms</a></li>
    </ol>
    <!--filter-->
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="executive.php">Available</a></li>
        <li class="breadcrumb-item"><a href="reserved-executive.php">Reserved</a></li>
        <li class="breadcrumb-item active" aria-current="page">Occupied</a></li>
    </ol>
</nav>
</div>

    <?php
$query_available_rooms = "SELECT rooms.room_number, rooms.status , room_types.name_of_room, room_types.image
    ,room_types.price ,packages.name_package, 
    packages.description
    FROM rooms
    LEFT JOIN room_types ON rooms.room_type_id = room_types.id
    LEFT JOIN packages ON rooms.room_type_id = packages.id
    WHERE rooms.status = 'occupied' AND room_types.name_of_room = 'executive'";
    $run_room = mysqli_query($conn,$query_available_rooms);
        ...
        if(mysqli_num_rows($run_room) > 0){
            ...
            <?php foreach($run_room as $row) {?>
                ...
                <img class="card-img-top" src="<?php echo "./Photos/hotel_rooms/" . $row ['image']?>" alt="economy room">
                    ...
                    <a class="btn btn-dark w-100 disabled" href="book-now.php?id=<?php echo $row ['room_number']?>">View</a>
                </div>
    <?php 
        }
    }
